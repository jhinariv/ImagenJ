...
public class TiffEncoder {
	...
	public TiffEncoder (FileInfo fi) {
		...
		//ij.IJMessage.log(imageOffset+", "+ifdSize+", "+bpsSize+", "+descriptionSize+", "+scaleSize+", "+colorMapSize+", "+nMetaDataEntries*4+", "+metaDataSize);
	}
	...
	public void write(OutputStream out) throws IOException {
		...
		if (nextIFD>0L) {
			...
			for (int i=2; i<=fi.nImages; i++) {
				...
				ij.IJMessage.log("Stack is larger than 4GB. Most TIFF readers will only open the first image. Use this information to open as raw:\n"+fi);
	}
