...
public class SurfacePlotter implements PlugIn {
	...
	public void run(String arg) {
		...
		if (img.getType()==ImagePlus.COLOR_RGB)
			{IJMessage.error("Surface Plotter", "Grayscale or pseudo-color image required"); return;}
		...
		if (stackFlags==PlugInFilter.DOES_STACKS && img.getStack().getSize()>1){
			...
			for (int i=2;i<=stackSource.getSize();i++) {
				IJMessage.showStatus("Drawing slice " + i + "..." + " (" + (100*(i-1)/stackSource.getSize()) + "% done)");
		...
		IJMessage.showStatus("Done in "+difference+" msec." );
	}
	...
	void drawAndLabelAxis(ImageProcessor ip, ImageProcessor ip2, Rectangle roi) {
		...
		//IJMessage.log("");
		//IJMessage.log(min+"  "+max+"  "+cal.getCValue((int)min)+"  "+cal.getCValue((int)max));
		//ip2.putPixelValue(0,0,0);
		//boolean zeroIsBlack
		//IJMessage.log(ip2.getPixelValue(+"  "+max+"  "+cal.getCValue((int)min)+"  "+cal.getCValue((int)max));
		if (cal.calibrated()) {
