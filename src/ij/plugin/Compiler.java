...
public class Compiler implements PlugIn, FilenameFilter {
	...
	public void run(String arg) {
		if (arg.equals("edit"))
			...
			if (arg!=null && arg.length()>0 && !arg.endsWith(".java"))
				IJMessage.error("Compiler", "File name must end with \".java\"");
			else
				compileAndRun(arg);
		}
	 }
	...
	void edit() {
		if (open("", "Open macro or plugin")) {
			Editor ed = (Editor)IJPlugin.runPlugIn(("ij.plugin.frame.Editor", "");
	...
	private void checkForUpdate(String plugin, String currentVersion) {
		...
		if (!f.exists() || !f.canWrite()) {
			if (IJDebugMode.debugMode) IJMessage.log("checkForUpdate: jar not found ("+plugin+")");
			return;
		}
		String version = null;
		try {
			Class c = IJPlugin.getClassLoader().loadClass("Compiler");
			version = "0.00a";
		...
		if (version==null) {
			if (IJDebugMode.debugMode) IJMessage.log("checkForUpdate: class not found ("+className+")");
			return;
		}
		if (version.compareTo(currentVersion)>=0) {
			if (IJDebugMode.debugMode) IJMessage.log("checkForUpdate: up to date ("+className+"  "+version+")");
			return;
		}
		boolean ok = Macro_Runner.downloadJar(plugin);
		if (IJDebugMode.debugMode) IJMessage.log("checkForUpdate: "+className+" "+version+" "+ok);
	}
	...
	boolean compile(String path) {
		IJMessage.showStatus("compiling "+path);
		...
		if (IJDebugMode.debugMode) {
			...
			IJMessage.log(builder.toString());
		}
	...
	void addJars(String path, StringBuffer sb) {
		...
		for (int i=0; i<list.length; i++) {
			...
			if (f2.isDirectory())
				...
				//IJMessage.log("javac classpath: "+path+list[i]);
			}
		}
	}
	...
	void showErrors(String s) {
		if (errors==null || !errors.isVisible()) {
			errors = (Editor)IJPlugin.runPlugIn(("ij.plugin.frame.Editor", "");
		...
		IJMessage.showStatus("done (errors)");
	}
			...
			if (okay) {
				if (msg.startsWith("Compile")) {
					if (!(lcName.endsWith(".java")||lcName.endsWith(".class"))) {
						IJMessage.error("File name must end with \".java\" or \".class\".");
						okay = false;
					}
					...
					IJMessage.error("File name must end with \".java\", \".txt\", \".ijm\", \".js\" or \".py\"");
					okay = false;
				}
			}
		} else {
...
class PlugInExecuter implements Runnable {
	...
	public void run() {
		IJ.resetEscape();
		IJPlugin.runPlugIn(("ij.plugin.ClassChecker", "");
		runCompiledPlugin(plugin);
	}

	void runCompiledPlugin(String className) {
		if (IJDebugMode.debugMode) IJMessage.log("Compiler: running \""+className+"\"");
		IJ.resetClassLoader();
		ClassLoader loader = IJPlugin.getClassLoader();
		...
		try {
			...
			if (IJDebugMode.debugMode) IJMessage.log("NoClassDefFoundError: "+err);
			...
			if (index>-1 && !className.contains(".")) {
				...
				if (className2.equals(className))
					IJMessage.error("Plugin not found: "+className2);
				else
...
abstract class CompilerTool {
	...
	public static class JavaxCompilerTool extends CompilerTool {
		...
		public boolean compile(List sources, List options, StringWriter log) {
			if (IJDebugMode.debugMode) IJMessage.log("Compiler: using javax.tool.JavaCompiler");
			try {
	...
	public static class LegacyCompilerTool extends CompilerTool {
		...
		public boolean compile(List sources, List options, StringWriter log) {
			if (IJDebugMode.debugMode) IJMessage.log("Compiler: using com.sun.tools.javac");
			try {
