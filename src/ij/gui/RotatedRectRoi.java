...
public class RotatedRectRoi extends PolygonRoi {
	...
	public RotatedRectRoi(int sx, int sy, ImagePlus imp) {
		...
		if (IJDebugMode.debugMode) IJMessage.log("RotatedRectRoi: "+(int)rectWidth+" "+pixels);
		if (rectWidth>pixels)
			rectWidth = pixels/3;
		setDrawOffset(false);
		bounds = null;
	...
	public void showStatus() {
		...
		IJMessage.showStatus("length=" + IJ.d2s(length)+", width=" + IJ.d2s(width)+", angle=" + IJ.d2s(angle));
	}
